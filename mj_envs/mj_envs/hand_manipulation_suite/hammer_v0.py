...
class HammerEnvV0(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, **kwargs):
        ...
        if self.control_mode == 'Torque':
            mujoco_env.MujocoEnv.__init__(self, curr_dir+'/assets/DAPG_hammer.xml', 5)  # frame_skip = 5, here are define frame_skip.
        elif self.control_mode == 'PID':
            mujoco_env.MujocoEnv.__init__(self, curr_dir+'/assets/DAPG_hammer.xml', 1)  # frame_skip = 1, here are define frame_skip.
    ...
    def get_obs(self):
        ...
        palm_pos = self.data.site_xpos[self.S_grasp_sid].ravel()
        target_pos = self.data.site_xpos[self.target_obj_sid].ravel()  # current nail position
        nail_impact = np.clip(self.sim.data.sensordata[self.sim.model.sensor_name2id('S_nail')], -1.0, 1.0)
        tool_pos = self.data.site_xpos[self.tool_sid].ravel() # current position of the hammer (hammer head)
        nail_impact = np.clip(self.sim.data.sensordata[self.sim.model.sensor_name2id('S_nail')], -1.0, 1.0)
        goal_pos = self.data.site_xpos[self.goal_sid].ravel() # goal position of the nail (always keep fixed)
        # original version
    ...
    def reset_model(self):
        self.sim.reset()
        target_bid = self.model.body_name2id('nail_board')
        # randomly generate the height of the target object(the z-value of the nail)
        # within distribution
        height = self.np_random.uniform(low=0.10, high=0.25)
        # out of distribution [0.05, 0.1] and [0.25, 0.3]
        # height = self.np_random.uniform(low=0.25, high=0.30)
        # height = self.np_random.uniform(low=0.05, high=0.10)
        # [0.05, 0.30] / [0.1, 0.25]
        # self.model.body_pos[target_bid,2] = self.np_random.uniform(low=0.1, high=0.25) (used in RL training)
        # height = self.np_random.uniform(low=0.1, high=0.15)
        # height = 0.1/0.15/0.2/0.25, work perfectly
        # if height = 0.05/0.3 (out of training distribution), RL work much better than Koopman (koopman has trouble)
        # if height = 0.11/0.12/0.13, nail is not fully driven in the wall
        # height = 0.3
        self.model.body_pos[target_bid,2] = height
        self.sim.forward()
        return self.get_obs(), height
