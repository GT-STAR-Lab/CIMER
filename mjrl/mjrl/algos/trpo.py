...
class TRPO(NPG):
    ...
    def train_from_paths(self, paths):
        ...
        # VPG
        ts = timer.time()
        vpg_grad = self.flat_vpg(observations, actions, advantages) # compute CPI_surrogate
        t_gLL += timer.time() - ts
        ...
        # This function is to adjust alpha
        for k in range(100):
        ...
        new_params = curr_params + alpha * npg_grad # To maximize the surrogate objective, so we add the gradient (Gradient ascent)
        self.policy.set_param_values(new_params, set_new=True, set_old=False)
        kl_dist = self.kl_old_new(observations, actions).data.numpy().ravel()[0]
        surr_after = self.CPI_surrogate(observations, actions, advantages).data.numpy().ravel()[0]
